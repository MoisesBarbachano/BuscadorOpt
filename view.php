...
if($first_time == 0 || isset($_POST['showNews'])  ){    
    if ($resultado = $mysqli->query("SELECT id,title,date,description,link FROM news_rss")) {
...
if (isset($_POST['clave'])) {
    if ($resultado = $mysqli->query("SELECT id,title,date,description,link FROM news_rss WHERE title LIKE '%" . $_POST['clave'] . "%'")) {
